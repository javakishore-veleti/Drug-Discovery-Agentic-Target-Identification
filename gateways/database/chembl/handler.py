"""
AgentCore Gateway Lambda target for MCP tool `chembl`.

Reuses shared adapter.search_chembl (AD-9 / AD-15).
"""
...
def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    logger.info(
        "chembl gateway invoke tool=%s event_keys=%s",
        tool_name,
        list(event or {}),
    )
    ...
    result = search_chembl(query, retmax=retmax)
