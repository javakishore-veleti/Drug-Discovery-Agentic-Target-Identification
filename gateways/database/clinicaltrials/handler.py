"""
AgentCore Gateway Lambda target for MCP tool `clinicaltrials`.

Reuses shared adapter.search_clinicaltrials (AD-9 / AD-15).
"""
...
def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    logger.info(
        "clinicaltrials gateway invoke tool=%s event_keys=%s",
        tool_name,
        list(event or {}),
    )
    ...
    result = search_clinicaltrials(query, retmax=retmax)
